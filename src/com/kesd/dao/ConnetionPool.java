...
public class ConnetionPool {
	...
	private static void initDriver() {
		Driver driver = null;
		try {
			driver = (Driver) Class.forName(HibSessionFactory.driver).newInstance();
	...
	private static ConnectionWrapper getNewConnection() {
		try {
			Connection con = DriverManager.getConnection(HibSessionFactory.url, HibSessionFactory.username, HibSessionFactory.password);
	...
	private static void clearClosedConnection() {
		long time = System.currentTimeMillis(); // sometimes user change system time,just return
	...
	public synchronized static void printDebugMsg(PrintStream out) {
		...
		msg.append("debug message in " + ConnetionPool.class.getName());
		msg.append("\r\n");
...
class ConnectionWrapper implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method m, Object[] args)	throws Throwable {
		...
		if (CLOSE_METHOD_NAME.equals(m.getName())) {
			ConnetionPool.pushConnectionBackToPool(this);
		} else {
