...
import com.kesd.comntpara.ComntParaBase.RTU_PARA;
...
public class ActMisDyGS {
	...
	private String MIS_DyQueryPower(String user_name, ComntParaBase.RTU_PARA rtu_para, ComntParaBase.OpDetailInfo op_detail, 
		...
		ComntMsgMisGS.YFFMISMSG_DY_QUERYPOWER_GS 			power_query  = new ComntMsgMisGS.YFFMISMSG_DY_QUERYPOWER_GS();
		ComntMsgMisGS.YFFMISMSG_DY_QUERYPOWER_RESULT_GS  	power_result = new ComntMsgMisGS.YFFMISMSG_DY_QUERYPOWER_RESULT_GS();
		...
		ret_val = ComntOperMisGS.DYQueryPower(user_name, i_user_data1, i_user_data2, rtu_para, p_mp_id, 
		...
		if (ret_val) {
			...
			if(power_result.retcode != ComntMsgMisGS.YFFMIS_MISOP_OKNO){
				...
				//MIS返回错误
					...
					misResult = "MIS用户查询失败, \n错误信息:" + err_str_1 + " , \nMIS错误信息:" + 
					power_result.retcode + ": " + ComntOperMisGS.GetMisErrStr(power_result.retcode) + " \n不能操作...";
				}
			}
			else {
	...
	//甘肃低压操作-缴费记录上传
	private String MIS_DyPay(String user_name, ComntParaBase.RTU_PARA rtu_para, ComntParaBase.OpDetailInfo op_detail, StringBuffer err_str_1, 
		...
		//发送
		ComntMsgMisGS.YFFMISMSG_DY_PAY_GS pay_req = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS();
		ComntMsgMisGS.YFFMISMSG_DY_PAY_RESULT_GS  pay_result = new ComntMsgMisGS.YFFMISMSG_DY_PAY_RESULT_GS();
		...
		double bcj_zje = p_paymoney,	//总金额 
			   bcj_qf = 0,				//实缴欠费 
			   bcj_ys = 0;				//实缴预收
		ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL []t_details_vect = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL[len];
		
		for(j = 0; j < len; j++){
			t_details_vect[j] = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL();
		...
		//未补齐以前电费
		if (j < len) {
			pay_req.details_vect = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL[j + 1];
			for (k = 0; k < (j + 1); k++) {
				pay_req.details_vect[k] = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL();
		...
		//已补齐以前电费
			...
			if (bcj_zje > 0.01) {
				pay_req.details_vect = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL[len + 1];
				for (k = 0; k < len; k++) {
					pay_req.details_vect[k] = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL();
					pay_req.details_vect[k].clone(t_details_vect[k]);
				}
		...
		//上传缴费记录
		task_result_detail.clear(); err_str_1.setLength(0);
		ret_val = ComntOperMisGS.DYPay(user_name, i_user_data1, i_user_data2, rtu_para, p_mp_id, 
		...
		//返回错误 读数据库
			...
			//容错处理
			Map<String, String> map = DBOperMis.loadNewJYffMisPayRecord(/*p_rtu_id, p_mp_id, */p_date, p_wasteno);
			if ((map != null) && 
		...
		if(payok_flag == false) {
			...
			//MIS返回错误
				...
				misResult = "上传缴费记录失败, \n错误信息:" + err_str_1 + " , \nMIS错误信息:" + 
				pay_result.retcode + ": " + ComntOperMisGS.GetMisErrStr(pay_result.retcode) + " \n请转入【补MIS缴费记录】页面...";
			}
		}
		else {
			misResult = "上传缴费记录成功。";
	...
	//甘肃低压操作-冲正
	private String MIS_DYRever(String user_name, ComntParaBase.RTU_PARA rtu_para, ComntParaBase.OpDetailInfo op_detail, StringBuffer err_str_1, 
								ComntParaBase.SALEMAN_TASK_RESULT_DETAIL task_result_detail) throws Exception {
		int i_user_data1 = CommBase.strtoi(userData1);	//RTU ID
		int i_user_data2 = CommBase.strtoi(userData2);	//USEROPER ID
		...
		//上传冲正记录
		ComntMsgMisGS.YFFMISMSG_DY_REVER_GS 	    rever_req  = new ComntMsgMisGS.YFFMISMSG_DY_REVER_GS();
		ComntMsgMisGS.YFFMISMSG_DY_REVER_RESULT_GS  rever_result = new ComntMsgMisGS.YFFMISMSG_DY_REVER_RESULT_GS();
		...
		ComntFunc.byteStringCutW(rever_req.czybh, ComntOperMisGS.OPERMAN_STRLEN);
		//对账批次
		...
		task_result_detail.clear(); err_str_1.setLength(0);
		ret_val = ComntOperMisGS.DYRever(user_name, i_user_data1, i_user_data2, rtu_para, p_mp_id, 
		...
		if (ret_val) {
			...
			Map<String, String> map_rev_record = DBOperMis.loadNewJYffMisReverRecord(p_date, p_rewasteno);
		...
		if (!rev_ok_flag) {
			...
			//MIS返回错误
				...
				misResult = "上传冲正记录失败, \n错误信息:" + err_str_1 + " , \nMIS错误信息:" + 
				rever_result.retcode + ": " + ComntOperMisGS.GetMisErrStr(rever_result.retcode) + " \n请转入【补MIS冲正记录】页面...";
			}
			return SDDef.FAIL;
		}
		...
		//MIS查询
		ComntMsgMisGS.YFFMISMSG_DY_QUERYPOWER_GS power_req = new ComntMsgMisGS.YFFMISMSG_DY_QUERYPOWER_GS();
		...
		if (ret_val == false || power_result.retcode != ComntMsgMisGS.YFFMIS_MISOP_OKNO || power_result.jezbs != power_result.details_vect.length) {
			if (task_result_detail.errcode == 0) {
				misResult = "MIS用户查询失败, \n错误信息:" + err_str_1 + " \n请转入【补MIS缴费记录】页面...";
			}
			else {
				misResult = "MIS用户查询失败, \n错误信息:" + err_str_1 + ", \nMIS错误信息:" + 
				task_result_detail.errcode + ": " + ComntOperMisGS.GetMisErrStr(task_result_detail.errcode) + " \n请转入【补MIS缴费记录】页面...";
			}
		...
		ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL []t_details_vect = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL[power_result.jezbs];
		...
		//已补齐以前电费
			...
			if (bcj_zje > 0.01) {
				pay_req.details_vect = new ComntMsgMisGS.YFFMISMSG_DY_PAY_GS.POWERPAY_DETAIL[power_result.jezbs + 1];
		...
		if (ret_val == false) {
			Map<String, String> map = DBOperMis.loadNewJYffMisPayRecord(/*p_rtu_id, p_mp_id,*/ 
									  CommBase.strtoi(record_thispay.get("op_date")), 
	...
	//甘肃低压操作-检查缴费
	private String MIS_DYCheckPay(String user_name, ComntParaBase.RTU_PARA rtu_para, ComntParaBase.OpDetailInfo op_detail, StringBuffer err_str_1, 
								ComntParaBase.SALEMAN_TASK_RESULT_DETAIL task_result_detail) throws Exception {
		...
		//读取MIS缴费操作记录
		Map<String, String> mis_payrcd = DBOperMis.loadNewJYffMisPayRecord(/*p_rtu_id, p_mp_id, */p_opdate, p_wasteno);
		if ((mis_payrcd != null) && 
		...
		*/
		...
		boolean pay_okflag = false;
		if (ret_val == false) {
			Map<String, String> map = DBOperMis.loadNewZYffMisPayRecord(/*p_rtu_id, p_mp_id,*/ 
									  p_opdate, p_wasteno);
			if ((map != null) && 
				(CommBase.strtoi(map.get("error_no")) == ComntMsgMisGS.YFFMIS_MISOP_OKNO)) {
				pay_okflag = true;
	...
	//甘肃低压操作-检查冲正
	private String MIS_DYCheckRever(String user_name, ComntParaBase.RTU_PARA rtu_para, ComntParaBase.OpDetailInfo op_detail, StringBuffer err_str_1, 
								ComntParaBase.SALEMAN_TASK_RESULT_DETAIL task_result_detail) throws Exception {
		...
		//读取MIS冲正操作记录
		Map<String, String> mis_reverrcd = DBOperMis.loadNewJYffMisReverRecord(/*p_rtu_id, p_mp_id, */p_opdate, p_wasteno);
		...
		//读取MIS缴费操作记录
		Map<String, String> mis_payrcd = DBOperMis.loadNewJYffMisPayRecord(/*p_rtu_id, p_mp_id, */p_opdate, p_rewasteno);
		//无记录不用再冲正
		if (mis_payrcd == null) {
			//更新本地数据库记录
			DBOperMis.updateJYffPayRecord(p_rtu_id, p_mp_id, p_opdate, p_optime, p_optype, true, true);
		...
		if (ret_val) {
			...
			Map<String, String> map_rev_record = DBOperMis.loadNewJYffMisReverRecord(p_opdate, p_wasteno);
			if ((map_rev_record != null) && 
				(CommBase.strtoi(map_rev_record.get("error_no")) == ComntMsgMisGS.YFFMIS_MISOP_OKNO)) {
				rev_ok_flag = true;
			}
