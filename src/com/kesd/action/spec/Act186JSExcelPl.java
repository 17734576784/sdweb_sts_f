...
public class Act186JSExcelPl {
	...
	private String  pagetype;		//页面类型   1:主站;2:金额;3:表底,4 无匹配档案。
...
//	private static final byte CONS_PAYSTOTHER	= 5;	//有匹配用户信息。费控状态不是本页面对应的类型（例如：主站费控页面：非金额费控、非主站算费）。
	...
	public static class ConsInfo {
		String 		cons_no   	= null;				//186户号
		String 		cons_name 	= "";				//186用户名
		
		double 		remain   	= -1;				//186余额               （对应文件的 “预收金额”）
		double[] 	bd_zyz   	= {-1.0,-1.0,-1.0};	//186表底正有总（对应文件的 “有功（总）-上次示数”）
		double[] 	bd_zyj   	= {-1.0,-1.0,-1.0};	//186表底正有尖（对应文件的 “有功（尖）-上次示数”）
		double[] 	bd_zyf  	= {-1.0,-1.0,-1.0};	//186表底正有峰（对应文件的 “有功（峰）-上次示数”）
		double[] 	bd_zyp   	= {-1.0,-1.0,-1.0};	//186表底正有平（对应文件的 “有功（平）-上次示数”）
		double[] 	bd_zyg   	= {-1.0,-1.0,-1.0};	//186表底正有谷（对应文件的 “有功（谷）-上次示数”）
		
		double[] 	bd_zwz   	= {-1.0,-1.0,-1.0};	//186表底正无总（对应文件的 “无功（总）-上次示数”）
		
		byte 		matchFlag  	= CONS_PARA;		//是否有匹配:0可以结算，1.其他费控类型，2 无匹配项--待处理

		int 		rtu_id		= -1;				//对应的终端ID
		short 		zjg_id		= -1;				//对应的总加组ID
		int 		zjg_idx		= -1;				//jsRtuZjgParas对应的index，取用电系统信息用。
	...
	static class JSRtuZjgPara
	{
		...
		public int 		cb_dayhour	= 0;
		
		public double 	othjs_money = 0.0;		//结算金额
		public double 	all_money	= 0.0;		//总金额
		public double 	alarm_val1	= 0.0;		//报警金额1
		public double 	alarm_val2	= 0.0;		//报警金额2
		public int 	  	buy_times	= 0;		//购电次数
		public double  	now_remain	= 0.0;		//当前余额
		public double   buy_dl		= 0.0;		//购电量
		public double   pay_bmc		= 0.0;		//表码差
		public double   shutdown_val= 0.0;		//断电止码
		public double   pay_add1	= 0.0;		//基本费
		public double   blv			= 1.0;		//倍率
		public int   	plus_time	= 0;		//脉冲宽度
		public byte   	feeproj_id[] 	= {-1,-1,-1};	//测量点费率方案id
		public byte   	feeproj_type[] 	= {-1,-1,-1};	//
		public long   	moneyLimit[] 	= {0,0,0};	//囤积限值
		public String  	feeproj_reteval[] 	= {"","",""};	//0正相 1反相
...
//			if(t_info.zjg_idx>=0){
//				byte chkcacl_type= 0;
//				byte chkfeectrl_type = 0;
//				switch(Integer.parseInt(pagetype)){
//				case 1:
//					chkcacl_type = 1;
//					chkfeectrl_type  = 1;break;
//				case 2:
//					chkcacl_type = 1;
//					chkfeectrl_type  = 0;break;
//				case 3:
//					chkcacl_type = 2;
//					chkfeectrl_type  = 0;break;
//				}
//				if(chkcacl_type != jsRtuZjgParas[t_info.zjg_idx].cacl_type || chkfeectrl_type != jsRtuZjgParas[t_info.zjg_idx].feectrl_type){
//					t_info.matchFlag = CONS_PAYSTOTHER;
//					continue;
//				}
//			}
		...
		try {
			...
			for(int i = startRow; i < rsRows; i++ ){
				...
				*/
				...
				//consList.get(cons_idx).cons_name = t_cons_desc;
			}
	...
	private  String toConsJsonStr(){
		...
		List<YffAlarmPara> yffalarm =Rd.getAlarmPara();
		
		for(int i = 0 ; i < consList.size(); i++){
			ConsInfo 		ci = consList.get(i) ;			
			JSRtuZjgPara	jr = (ci.zjg_idx >= 0 ? jr = jsRtuZjgParas[ci.zjg_idx] : null );
			
			sbf.append(SDDef.JSONLBRACES); 
			sbf.append(SDDef.JSONQUOT + "id" + SDDef.JSONQACQ + i + SDDef.JSONDATA);	
			sbf.append(SDDef.JSONQUOT + i + SDDef.JSONCCM);//1
			//SG186参数
			sbf.append(SDDef.JSONQUOT + ci.cons_name 	+ SDDef.JSONCCM);//2用户名称
			sbf.append(SDDef.JSONQUOT + ci.cons_no  	+ SDDef.JSONCCM);//3户号
			sbf.append(SDDef.JSONQUOT + ci.remain  		+ SDDef.JSONCCM);//4预收金额
			String bd = ci.bd_zyz[0] + "_" + ci.bd_zyz[1] + "_" + ci.bd_zyz[2]; 
			sbf.append(SDDef.JSONQUOT + bd 			+ SDDef.JSONCCM);//5有功总
			bd = ci.bd_zwz[0] + "_" + ci.bd_zwz[1] + "_" + ci.bd_zwz[2]; 
			sbf.append(SDDef.JSONQUOT + bd  			+ SDDef.JSONCCM);//6无功总
			sbf.append(SDDef.JSONQUOT + ci.matchFlag	+ SDDef.JSONCCM);//7匹配档案
			//售电子系统参数
			if(jr!=null){
				sbf.append(SDDef.JSONQUOT + jr.cons_name 	+ SDDef.JSONCCM);//8用户名称
				sbf.append(SDDef.JSONQUOT + Rd.getDict(Dict.DICTITEM_YFFCUSSTATE, jr.cus_state)	+ SDDef.JSONCCM);//9用户状态-文字
				sbf.append(SDDef.JSONQUOT + jr.othjs_money 	+ SDDef.JSONCCM);//10结算金额--主站、金额
				sbf.append(SDDef.JSONQUOT + jr.alarm_val1 	+ SDDef.JSONCCM);//11报警止码--表底
				sbf.append(SDDef.JSONQUOT + jr.all_money 	+ SDDef.JSONCCM);//12总金额
				sbf.append(SDDef.JSONQUOT + jr.alarm_val1  	+ SDDef.JSONCCM);//13报警金额1（元）--主站、金额
				sbf.append(SDDef.JSONQUOT + jr.alarm_val2 	+ SDDef.JSONCCM);//14报警金额2（元）--主站、金额
				sbf.append(SDDef.JSONQUOT + jr.buy_times 	+ SDDef.JSONCCM);//15购电次数
				sbf.append(SDDef.JSONQUOT + jr.now_remain 	+ SDDef.JSONCCM);//16当前余额--主站、金额
				sbf.append(SDDef.JSONQUOT + jr.buy_dl		+ SDDef.JSONCCM);//17购电量--表底
				sbf.append(SDDef.JSONQUOT + jr.pay_bmc		+ SDDef.JSONCCM);//18表码差--表底
				sbf.append(SDDef.JSONQUOT + jr.shutdown_val + SDDef.JSONCCM);//19断电止码--表底
				sbf.append(SDDef.JSONQUOT + jr.shutdown_val	+ SDDef.JSONCCM);//20断电表底--表底
				sbf.append(SDDef.JSONQUOT + "" 				+ SDDef.JSONCCM);//21当前表底--表底
				sbf.append(SDDef.JSONQUOT + "" 				+ SDDef.JSONCCM);//22操作结果
				//隐藏域 第17列开始
				sbf.append(SDDef.JSONQUOT + jr.cacl_type 	+ SDDef.JSONCCM);//23
				sbf.append(SDDef.JSONQUOT + jr.feectrl_type	+ SDDef.JSONCCM);//24
				sbf.append(SDDef.JSONQUOT + jr.yffalarm_id	+ SDDef.JSONCCM);//25
				sbf.append(SDDef.JSONQUOT + jr.cb_dayhour	+ SDDef.JSONCCM);//26
				sbf.append(SDDef.JSONQUOT + jr.cus_state	+ SDDef.JSONCCM);//27用户状态-值
				sbf.append(SDDef.JSONQUOT + jr.pay_type 	+ SDDef.JSONCCM);//28
				sbf.append(SDDef.JSONQUOT + jr.cons_id 		+ SDDef.JSONCCM);//29//结算用
				sbf.append(SDDef.JSONQUOT + jr.shutdown_val + SDDef.JSONCCM);//30//结算用
				sbf.append(SDDef.JSONQUOT + jr.blv 			+ SDDef.JSONCCM);//31倍率
				sbf.append(SDDef.JSONQUOT + jr.plus_time	+ SDDef.JSONCCM);//32脉冲宽度
				sbf.append(SDDef.JSONQUOT + jr.pay_add1 	+ SDDef.JSONCCM);//33基本费
				  
				//报警方案
				YffAlarmPara alm = Rd.getAlarmPara(yffalarm, jr.yffalarm_id);
				if(alm != null){
					sbf.append(SDDef.JSONQUOT + alm.getAlarm1() + SDDef.JSONCCM);//34
					sbf.append(SDDef.JSONQUOT + alm.getAlarm2() + SDDef.JSONCCM);//35
					sbf.append(SDDef.JSONQUOT + alm.getType() + SDDef.JSONCCM);//36报警方式
				}else{
					sbf.append(SDDef.JSONQUOT +  "-1"  	+ SDDef.JSONCCM);//34
					sbf.append(SDDef.JSONQUOT +  "-1" 	+ SDDef.JSONCCM);//35
					sbf.append(SDDef.JSONQUOT +  "-1" 	+ SDDef.JSONCCM);//36
				}
				
				//费率方案
				String feeproj_id="",feeproj_type="",moneyLimit="", feeproj_reteval="";
				for(int ii = 0; ii < jr.clp_num; ii++ ){
					if( jr.feeproj_id[ii] < 0 )continue;
					YffRatePara rate= Rd.getYffRate(yffrate, jr.feeproj_id[ii]);
					feeproj_id+="_" + jr.feeproj_id[ii];
					feeproj_type+="_" + rate.getFeeType();
					moneyLimit+="_" + rate.getMoneyLimit();
					feeproj_reteval+="_" +Rd.getYffRateVal(rate);
				}
				for(int ii = jr.clp_num; ii < 3; ii++ ){
					feeproj_id+="_" + "-1";
					feeproj_type+="_" + "-1";
					moneyLimit+="_" + "-1";
					feeproj_reteval+="_" +"";
				}
				if(feeproj_id != "" )		feeproj_id 		= feeproj_id.substring(1);
				if(feeproj_type != "" )		feeproj_type 	= feeproj_type.substring(1);
				if(moneyLimit != "" )		moneyLimit 		= moneyLimit.substring(1);
				if(feeproj_reteval != "" )	feeproj_reteval = feeproj_reteval.substring(1);
				sbf.append(SDDef.JSONQUOT + feeproj_id  	+ SDDef.JSONCCM);//37
				sbf.append(SDDef.JSONQUOT + feeproj_type 	+ SDDef.JSONCCM);//38
				sbf.append(SDDef.JSONQUOT + moneyLimit 		+ SDDef.JSONCCM);//39
				sbf.append(SDDef.JSONQUOT + feeproj_reteval + SDDef.JSONCCM);//40
				
			}else{
				sbf.append(SDDef.JSONQUOT + "--" 	+ SDDef.JSONCCM);//8用户名称
				sbf.append(SDDef.JSONQUOT + "--"  	+ SDDef.JSONCCM);//9用户状态
				sbf.append(SDDef.JSONQUOT + "--"  	+ SDDef.JSONCCM);//10结算金额--主站、金额                   
				sbf.append(SDDef.JSONQUOT + "--"  	+ SDDef.JSONCCM);//11报警止码--表底                      
				sbf.append(SDDef.JSONQUOT + "--"  	+ SDDef.JSONCCM);//12总金额                           
				sbf.append(SDDef.JSONQUOT + "--"   	+ SDDef.JSONCCM);//13报警金额1（元）--主站、金额               
				sbf.append(SDDef.JSONQUOT + "--"  	+ SDDef.JSONCCM);//14报警金额2（元）--主站、金额               
				sbf.append(SDDef.JSONQUOT + "--" 	+ SDDef.JSONCCM);//15购电次数                          
				sbf.append(SDDef.JSONQUOT + "--"  	+ SDDef.JSONCCM);//16当前余额--主站、金额                   
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//17购电量                           
				sbf.append(SDDef.JSONQUOT + ""  	+ SDDef.JSONCCM);//18表码差                           
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//19断电止码                          
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//20断电表底                          
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//21当前表底                          
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//22操作结果                          
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//23计费方式
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//24费控方式
				sbf.append(SDDef.JSONQUOT + "-1"  	+ SDDef.JSONCCM);//25报警方案
				sbf.append(SDDef.JSONQUOT + "-1" 	+ SDDef.JSONCCM);//26
				sbf.append(SDDef.JSONQUOT + "-1" 	+ SDDef.JSONCCM);//27
				sbf.append(SDDef.JSONQUOT + "-1" 	+ SDDef.JSONCCM);//28
				sbf.append(SDDef.JSONQUOT + "-1" 	+ SDDef.JSONCCM);//29
				sbf.append(SDDef.JSONQUOT + "-1" 	+ SDDef.JSONCCM);//30
				sbf.append(SDDef.JSONQUOT + "-1"	+ SDDef.JSONCCM);//31倍率
				sbf.append(SDDef.JSONQUOT + "-1"	+ SDDef.JSONCCM);//32脉冲宽度
				sbf.append(SDDef.JSONQUOT + "-1"	+ SDDef.JSONCCM);//33基本费
				sbf.append(SDDef.JSONQUOT + "-1"  	+ SDDef.JSONCCM);//34报警值1
				sbf.append(SDDef.JSONQUOT + "-1" 	+ SDDef.JSONCCM);//35 报警值2
				sbf.append(SDDef.JSONQUOT + "-1" 	+ SDDef.JSONCCM);//36 报警方式
				sbf.append(SDDef.JSONQUOT + ""  	+ SDDef.JSONCCM);//37feeproj_id
				sbf.append(SDDef.JSONQUOT + ""		+ SDDef.JSONCCM);//38feeproj_type
				sbf.append(SDDef.JSONQUOT + ""		+ SDDef.JSONCCM);//39moneyLimit
				sbf.append(SDDef.JSONQUOT + "" 		+ SDDef.JSONCCM);//40feeproj_reteval
				
			}
			
			sbf.append(SDDef.JSONQUOT + ci.rtu_id  		+ SDDef.JSONCCM);//41
			sbf.append(SDDef.JSONQUOT + ci.zjg_id  		+ SDDef.JSONCCM);//42
			sbf.append(SDDef.JSONQUOT + ci.mp_num 		+ SDDef.JSONCCM);//43
			sbf.append(SDDef.JSONQUOT + (ci.mp_ids[0] + "_" + ci.mp_ids[1] + "_" + ci.mp_ids[2]) + SDDef.JSONCCM);		//44mp_ids
			sbf.append(SDDef.JSONQUOT + (ci.zf_flag[0] + "" + ci.zf_flag[1] + "" + ci.zf_flag[2]) + SDDef.JSONCCM);	//45正反相
			sbf.append(SDDef.JSONQUOT + ci.cacl_type 	+ SDDef.JSONCCM);//46
			sbf.append(SDDef.JSONQUOT + ci.fee_type 	+ SDDef.JSONCCM);//47
			
			bd = ci.bd_zyj[0] + "_" + ci.bd_zyj[1] + "_" + ci.bd_zyj[2];
			sbf.append(SDDef.JSONQUOT + bd		   + SDDef.JSONCCM);	//48有功尖
...
//						 "order by c.rtu_id, c.zjg_id";
		...
		sql_cmd.append("select a.id cons_id, a.busi_no, a.describe cons_desc, c.rtu_id, c.zjg_id, c.clp_num, c.clp_ids, c.zf_flag, d.cacl_type, d.feectrl_type,d.pay_type,d.yffalarm_id,d.cb_dayhour,d.pay_add1,d.plus_time,d.feeproj_id,d.feeproj1_id,d.feeproj2_id,e.cus_state,e.othjs_money,e.all_money,e.alarm_val1,e.alarm_val2,e.buy_times,e.now_remain,e.buy_dl,e.pay_bmc,e.shutdown_val ");
		...
		try {
			...
			while(rs.next()){
				...
				ret_para[idx].cons_id  = rs.getString("cons_id");
				ret_para[idx].cons_name= rs.getString("cons_desc");
				...
				ret_para[idx].othjs_money 	= Double.parseDouble(CommBase.CheckNum(rs.getString("othjs_money")));
				ret_para[idx].all_money 	= Double.parseDouble(CommBase.CheckNum(rs.getString("all_money")));
				ret_para[idx].alarm_val1 	= Double.parseDouble(CommBase.CheckNum(rs.getString("alarm_val1")));
				ret_para[idx].alarm_val2 	= Double.parseDouble(CommBase.CheckNum(rs.getString("alarm_val2")));
				ret_para[idx].cb_dayhour 	= Integer.parseInt(CommBase.CheckNum(rs.getString("cb_dayhour")));
				ret_para[idx].buy_times 	= Integer.parseInt(CommBase.CheckNum(rs.getString("buy_times")));
				ret_para[idx].yffalarm_id 	= Byte.parseByte(CommBase.CheckNum(rs.getString("yffalarm_id")));
				ret_para[idx].now_remain  	= Double.parseDouble(CommBase.CheckNum(rs.getString("now_remain")));
				ret_para[idx].buy_dl		= Double.parseDouble(CommBase.CheckNum(rs.getString("buy_dl")));
				ret_para[idx].pay_bmc		= Double.parseDouble(CommBase.CheckNum(rs.getString("pay_bmc")));
				ret_para[idx].shutdown_val 	= Double.parseDouble(CommBase.CheckNum(rs.getString("shutdown_val")));
				ret_para[idx].pay_add1		= Double.parseDouble(CommBase.CheckNum(rs.getString("pay_add1")));
				ret_para[idx].plus_time		= Integer.parseInt(CommBase.CheckNum(rs.getString("plus_time")));
				
				if (ret_para[idx].clp_num > 0) {
					
					//查询倍率
					String sql_cmd2 = "select pt_ratio, ct_ratio from mppara where rtu_id=" + ret_para[idx].rtu_id + " and id=" + ret_para[idx].mp_ids[0];
					rs1 = j_dao.executeQuery(sql_cmd2);
					try{
						if (rs1.next()) {
							ret_para[idx].blv = rs1.getDouble("pt_ratio") * rs1.getDouble("ct_ratio");						
						}
					}catch(Exception e) {
						ret_para[idx].blv = 1;
					}
					...
					//费率方案
					if(rs.getString("feeproj_id") != null) ret_para[idx].feeproj_id[0] = Byte.parseByte(CommBase.CheckNum(rs.getString("feeproj_id")));
					if(rs.getString("feeproj1_id") != null) ret_para[idx].feeproj_id[1] = Byte.parseByte(CommBase.CheckNum(rs.getString("feeproj1_id")));
					if(rs.getString("feeproj2_id") != null) ret_para[idx].feeproj_id[2] = Byte.parseByte(CommBase.CheckNum(rs.getString("feeproj2_id")));					
				}				
	...
	private JSMeterPara[] loadJsMeterPara()
	{
		...
		sql_cmd.append("select c.busi_no cons_no,c.describe cons_desc, b.rtu_id, b.mp_id, b.made_no from rtupara as a, meterpara as b,conspara c "); 
		...
		try {
			...
			while(rs.next()){
				...
				ret_para[idx].cons_name  = CommBase.CheckString(rs.getString("cons_desc"));
	...
	private String getOneCons(){
		...
		j_obj.put("jbf", 		CommBase.CheckString(map.get("pay_add1")));	//基本费
