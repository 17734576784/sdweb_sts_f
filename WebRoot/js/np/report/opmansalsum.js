...
function initgrid(){
	var gridHeader = "序号,营业点,操作员,售电次数,电费(元),#cspan,#cspan,#cspan,&nbsp;";
	...
	$("#filename").val(encodeURI("农排操作员售电汇总表"));
...
function search(){
	...
	$.post(def.basePath + "ajaxnp/actReport!opmanSalsumReport.action",{result : param},function(data){
		loading.loaded();
		if(data.result != ""){
