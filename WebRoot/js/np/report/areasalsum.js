...
function initgrid(){
	var gridHeader = "序号,营业点,片区名称,售电次数,电费(元),#cspan,#cspan,#cspan,&nbsp;";
	...
	mygrid.setInitWidths("50,140,140,80,120,120,120,120,*");
	mygrid.setColAlign("center,left,left,right,right,right,right,right");
	...
	$("#filename").val(encodeURI("区域时间段片区售电汇总表"));
...
function search(){
	...
	var param = '{sdate:"'+sdate+'",edate:"'+edate+'",org:"'+$("#org").val() + '",area:"'+ $("#area").val() + '"}';
	
	loading.loading();
	
	$.post(def.basePath + "ajaxnp/actReport!areaSalSumReport.action",{result : param},function(data){
		loading.loaded();
		if(data.result != ""){
