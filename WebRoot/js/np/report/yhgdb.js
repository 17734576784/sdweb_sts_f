...
function initgrid(){
	...
	var gridHeader = "序号,客户编号,客户名称,卡号,缴费次数,电费(元),#cspan,#cspan,#cspan,&nbsp;";
	var attachheader = "#rspan,#rspan,#rspan,#rspan,#rspan,缴费金额,追补金额,结算金额,总金额,#rspan";
	var datatype = "int,str,str,str,int,0.000,0.000,0.000,0.000";
	mygrid.setHeader(gridHeader);
	mygrid.attachHeader(attachheader);
	mygrid.setInitWidths("50,120,160,130,80,120,120,120,120,*");
	mygrid.setColAlign("center,left,left,left,right,right,right,right,right");
	mygrid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro");
	mygrid.init();
	mygrid.setSkin("modern");
	
	var empty = "&nbsp;";
	var footer = "<div id='hj'>合计：</div>,#cspan,#cspan,#cspan,<div id='jfcs'>"+empty+"</div>,<div id='jfje'>"+empty+"</div>,<div id='zbje'>"+empty+"</div>,<div id='jsje'>"+empty+"</div>,<div id='zje'>"+empty+"</div>,&nbsp;,&nbsp;,#cspan";
	...
	$("#filename").val(encodeURI("农排用户购电表"));
...
function search(){
	...
	var param = '{sdate:"'+sdate+'",edate:"'+edate+'",yhbh:"'+$("#yhbh").val()+'",org:"'+$("#org").val()+'",area:"'+$("#area").val()+'"}';
	
	loading.loading();
	
	$.post(def.basePath + "ajaxnp/actReport!yhgdReport.action",{result : param},function(data){
		loading.loaded();
		if(data.result != ""){
			...
			var footer = '{rows:[{data:["'+$("#hj").html()+',0-3","'+$("#jfcs").html()+'","'+$("#jfje").html()+'","'+$("#zbje").html()+'","'+$("#jsje").html()+'","'+$("#zje").html()+'"]}]}';
