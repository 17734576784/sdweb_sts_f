...
function wrtCard_chgmeter1(gsm){	
	...
	params.card_type  	= SDDef.NPCARD_OPTYPE_OPEN;//开户
	...
	$.post( def.basePath + "ajaxoper/actOperDy!taskProc.action", 		//后台处理程序
		...
		function(data) {//回传函数
			...
			if (data.result == "success") {
				//向MIS系统缴费
				if(parseFloat(params.pay_money) > 0 && misUseflag == "true" && misOkflag == "true") {
					...
					mis_para.vlength = misDetailsvect.length;
				...
				window.top.WebPrint.setYffDataOperIdx2params(data.dyOperChangeMeter,window.top.WebPrint.nodeIdx.npcard);//打印用的参数
			}
			else {
...
//	
//	getYffRecs();
//	var new_buy_times 	= B_TIME_KH;
//	var new_last_jfje	= $("#zje").html()===""?0:$("#zje").html();
//	
//	$("#buy_times").html(new_buy_times);
...
//function check(){
//	if(!checkBDZ("_o") || !checkBDZ("_n")){
//		return false;
//	}
//	if (!isZfDbl("zbje_val", "剩余金额", false)) {
//		return false;
//	}
//	if(!isDbl("jfje_val","缴费金额", 0, 1000000, true)) {
//		return false;
//	}
//
//	return true;
...
//	if(rtnValue.userno != consNo &&  tmp != consNo  && tmp != meterNo && meterNo != rtnValue.esamno) 
//	{
//		alert("卡内信息与户号表号与数据库中不匹配。请确认！如果确实要写该卡，请先清空卡。")
//		return false;
//	}
...
//	var datas = {};
//	datas.mpid = $("#ghdb").val();
//	datas.rtuid = $("#rtu_id").val();
//	var json_str = jsonString.json2String(datas);
//	
//	$.post( def.basePath + "ajaxdyjc/actChgMeter!getCTbyIDs.action",{result:json_str},function(data) {			    	//回传函数
//		if (data.result != "") {
//			var json = eval('(' + data.result + ')');
//			$("#ctfz").val((json.ct_n==0 ? 1:json.ct_n));
//			$("#ctfm").val((json.ct_d==0 ? 1:json.ct_d));
//			$("#ctbb").html((json.ct_r==0 ? 1:json.ct_r));
//		}
//		else {
//			$("#ctfz").val(1);
//			$("#ctfm").val(1);
