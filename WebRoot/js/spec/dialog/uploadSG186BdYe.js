...
var params = window.dialogArguments;//""单个结算，1/2/3  批量结算时的页面类型   1:主站;2:金额;3:表底。	
...
function doPreview(){
	...
	if(params==""){//结算补差
		document.forms[0].action = basePath + "excel/SG186JSExcelGy.action?params=" + qx_params;	
	}else{
		document.forms[0].action = basePath + "excel/SG186JSExcelGyPl.action?params=" + qx_params + "&pagetype=" + params;
	}
