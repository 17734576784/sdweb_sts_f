...
function readcard(){
	...
	setTimeout("window.top.card_comm.readCardSearchGy(" + ComntUseropDef.YFF_GYOPER_PAY + ")", 10);
...
function setCardCons(){
	...
	switch(meterType){
	case window.top.SDDef.YFF_METER_TYPE_ZNK:
		setCardCons_ZNK09(meterType,cardInfo);
		break;
	case window.top.SDDef.YFF_METER_TYPE_ZNK2:
		setCardCons_ZNK13(meterType,cardInfo);
		break;
	
	case window.top.SDDef.YFF_METER_TYPE_6103:
		setCardCons_6103(meterType,cardInfo);
		break;
	default:
		alert("卡类型错误！");
		break;
...
function setCardCons_ZNK09(meterType,cardInfo){
	var rtn_json = {};
	rtn_json.type = window.top.SDDef.YFF_METER_TYPE_ZNK;
	...
	//rtn_json.userType_back     = cardInfo[24];
	...
	//卡内用户信息
	$("#tabinfo2").attr("style","display:block");
	$("#tabinfo3").attr("style","display:block");
	$("#tabinfo4").attr("style","display:none");
	$("#tabinfo5").attr("style","display:none");
	$("#tabinfo6").attr("style","display:none");
	$("#tabinfo7").attr("style","display:none");
...
function setCardCons_ZNK13(meterType,cardInfo){
	var rtn_json = {};
	...
	$("#tabinfo4").attr("style","display:none");
	$("#tabinfo5").attr("style","display:none");
	$("#tabinfo6").attr("style","display:block");
	$("#tabinfo7").attr("style","display:block");
...
function setCardCons_6103(meterType,cardInfo){
	var rtn_json = {};
	rtn_json.type = window.top.SDDef.YFF_METER_TYPE_6103;
	rtn_json.meterType = "科林6103";
	if(cardInfo[1]=="0"){
		rtn_json.cardType = "开户卡";
	}else if(cardInfo[1]=="1"){
		rtn_json.cardType = "购电卡";
	}
	rtn_json.userNo    = cardInfo[2];
	rtn_json.alarm1    = parseFloat(cardInfo[3])/100;
	rtn_json.alarm2    = parseFloat(cardInfo[4])/100;
	rtn_json.buy_dl    = parseFloat(cardInfo[5])/100;
	rtn_json.buy_num   = cardInfo[6];
	rtn_json.limit_val = parseFloat(cardInfo[7])/100;
	rtn_json.bl	       = cardInfo[8];
	...
	rtn_json.userNo_back     = cardInfo[13];
	rtn_json.remain_dl_back  = parseFloat(cardInfo[15])/100;
	rtn_json.total_dl_back   = parseFloat(cardInfo[16])/100;
	rtn_json.bcbuy_dl_back   = parseFloat(cardInfo[17])/100;
	rtn_json.buy_num_back    = cardInfo[18];
	rtn_json.ffck_num_back   = cardInfo[20];
	rtn_json.imp_back		 = cardInfo[22];
	rtn_json.alarm1_back     = parseFloat(cardInfo[23])/100;
	rtn_json.alarm2_back     = parseFloat(cardInfo[24])/100;	
	
	var year  = parseInt(cardInfo[25]/10000);
	var month = parseInt((cardInfo[25]%10000)/100);
	var day   = parseInt((cardInfo[25]%10000)%100);
	var hour  = parseInt(cardInfo[26]/10000);
	var min   = parseInt((cardInfo[26]%10000)/100);
	var sec   = parseInt((cardInfo[26]%10000)%100);
	
	rtn_json.fxDate_back = "20"+year+"年"+month+"月"+day+"日";
	rtn_json.fxTime_back = hour+"时"+min+"分"+sec+"秒";
	
	rtn_json.bl_back         = cardInfo[28];
	if(rtn_json.buy_num == rtn_json.buy_num_back){
		rtn_json.cardState = "已透支";
	}else{
		rtn_json.cardState = "有余额";
	}
	if(rtn_json.userNo_back == "FFFFFFFFFFFFFFFFFFFF") {
		rtn_json.write_back_flag = "否";
	}
	else {
		rtn_json.write_back_flag = "是";
	...
	$("#tabinfo2").attr("style","display:none");
	$("#tabinfo3").attr("style","display:none");
	$("#tabinfo4").attr("style","display:block");
	$("#tabinfo5").attr("style","display:block");
	$("#tabinfo6").attr("style","display:none");
	$("#tabinfo7").attr("style","display:none");
	
	$("#userno_03"   ).html(rtn_json.userNo);
	$("#metertype_03").html(rtn_json.meterType);
	$("#bl_03" 		 ).html(rtn_json.bl);
	$("#card_type_03").html(rtn_json.cardType);
	$("#alarm_1_03"  ).html(rtn_json.alarm1);
	$("#alarm_2_03"  ).html(rtn_json.alarm2);
	$("#buy_num_03"  ).html(rtn_json.buy_num);
	$("#buy_dl_03"   ).html(rtn_json.buy_dl);
	$("#limit_val_03").html(rtn_json.limit_val);
	
	//以下为返写信息
	//返写信息
	if(rtn_json.write_back_flag == "否"){
		$("#write_back_flag_03").html("否");
		$("#userno_03_back"    ).html("");
		$("#bl_03_back"        ).html("");
		$("#feetype_03_back"   ).html("");
		$("#sydl_03_back"      ).html("");
		$("#imp_03_back"       ).html("");
		$("#ljgdl_03_back"     ).html("");
		$("#bcgdl_03_back"     ).html("");
		...
		$("#write_back_flag_03").html("是");
		$("#userno_03_back" ).html(rtn_json.userNo_back);
		$("#bl_03_back"     ).html(rtn_json.bl_back);
		$("#feetype_03_back").html(rtn_json.feeType);
		$("#sydl_03_back"   ).html(rtn_json.remain_dl_back);
		$("#imp_03_back"    ).html(rtn_json.imp_back);
		$("#ljgdl_03_back"  ).html(rtn_json.total_dl_back);
		$("#bcgdl_03_back"  ).html(rtn_json.bcbuy_dl_back);
		$("#buynum_03_back" ).html(rtn_json.buy_num_back);
		$("#alarm1_03_back" ).html(rtn_json.alarm1_back);
		$("#alarm2_03_back" ).html(rtn_json.alarm2_back);
		$("#fxdata_03_back" ).html(rtn_json.fxDate_back);
		$("#fxtime_03_back" ).html(rtn_json.fxTime_back);
	}
}
