...
$(document).ready(function(){
	...
	$("#syje").keyup(function(){if(isNaN(this.value)){this.value = "";return}; if(this.value=="")return;calcu_bd();});
	$("#alarm_bd").keyup(function(){if(isNaN(this.value))this.value = "";});
...
function calcu_bd(){
	...
	var tt_num 	= parseFloat($("#syje").val() === "" ? 0 : $("#syje").val());
	var buy_dl 	= 0, buy_bmc = 0, new_shutdownbd = 0, alarm_val = 0;
	
	var zbd = 0.0;
	zbd = zbd + parseFloat(isNaN(rtnBDNew.zbd0) ? 0 : rtnBDNew.zbd0);
	zbd = zbd + parseFloat(isNaN(rtnBDNew.zbd1) ? 0 : rtnBDNew.zbd1);
	zbd = zbd + parseFloat(isNaN(rtnBDNew.zbd2) ? 0 : rtnBDNew.zbd2);

	var pt_ratio = 1, ct_ratio = 1;
	
	if($("#chg_type").val() == SDDef.YFF_CHGMETER_CT){
		ct_ratio = $("#ct").html() == "" ? 1 : $("#ct").html();
	}
	else{
		var m_id = $("#chg_meter").val();
		for ( var i = 0; i < ptct_info.rows.length; i++) {
			if(ptct_info.rows[i].value == m_id){
				ct_ratio = ptct_info.rows[i].ct;
				break;
			}
		}
	}
	
	if($("#chg_type").val() == SDDef.YFF_CHGMETER_PT){
		pt_ratio = $("#pt").html() == "" ? 1 : $("#pt").html();
	}
	else{
		var m_id = $("#chg_meter").val();
		for ( var i = 0; i < ptct_info.rows.length; i++) {
			if(ptct_info.rows[i].value == m_id){
				pt_ratio = ptct_info.rows[i].pt;
				break;
			}
		}
	}
	...
	buy_dl  = tt_num / dj;
	buy_bmc = tt_num / (dj * pt_ratio * ct_ratio);
	new_shutdownbd = tt_num / (dj * pt_ratio * ct_ratio) + zbd;
	if(new_shutdownbd < 0) new_shutdownbd = 0;
	
	$("#buy_dl").html(round(buy_dl, def.point));
	$("#bmc").html(round(buy_bmc, 	def.point));
	$("#shutdown_bd").html(round(new_shutdownbd, def.point));
	
	if(alarm_type == 0) {		//固定值方式
		alarm_val = new_shutdownbd - rtnValue.yffalarm_alarm1;
		if (alarm_val <= zbd) alarm_val = zbd + buy_bmc * 0.7;
	}
	else {
		alarm_val = new_shutdownbd - buy_bmc * rtnValue.yffalarm_alarm1 * 0.01;
	}
	$("#alarm_bd").val(round(alarm_val, def.point));
}
...
function setTextDisabled(mode){	//设置文本框状态（disabled/enabled）
	if(!mode){
		...
		$("#chg_type").val(SDDef.YFF_CHGMETER_MT);
		chg_type(SDDef.YFF_CHGMETER_MT);
	}
...
function selcons(){//检索
	...
	$("#yffalarm_alarm1").val(rtnValue.yffalarm_alarm1);
	$("#yffalarm_alarm2").val(rtnValue.yffalarm_alarm2);
...
function chgMeter(){
	...
	params.alarmmoney  		= 0;
	params.buydl  			= $("#buy_dl").html();
	params.paybmc  			= $("#bmc").html();
	params.shutbd  			= $("#shutdown_bd").html();	//缴费和换表使用的字段不同，所以对两个分别赋值。
	params.shutdown_bd		= $("#shutdown_bd").html();
	params.alarmbd  		= $("#alarm_bd").val();
