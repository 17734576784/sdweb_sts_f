...
function selcons(){//检索
	var rtnValue1 = doSearch("zz",ComntUseropDef.YFF_GYOPER_RESTART, rtnValue);
	if(!rtnValue1){
		return;
	}
	rtnValue = rtnValue1;
		...
		function(data) {			    	//回传函数
			...
			if (data.result == "success") {		
				...
				$("#btnPrt").attr("disabled",false);
   	 			window.top.WebPrint.setYffDataOperIdx2params(data.gyOperReStart,window.top.WebPrint.nodeIdx.gymain);//打印用的参数
