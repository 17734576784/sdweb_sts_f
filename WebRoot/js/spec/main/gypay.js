...
function selcons(){//检索
	var tmp = doSearch("zz",ComntUseropDef.YFF_GYCOMM_PAY, rtnValue);
	if(!tmp){
		return;
	}
	rtnValue = tmp;
...
function check(){
	...
	if ((window.top.gynegativemoney.gymain.remainflag == 0) &&
		(parseFloat($("#zje").val()) + parseFloat($("#dqye").html()) < 0)) {
		alert("最新余额不能为负数");
		return false;
	}
		...
		function(data) {			    	//回传函数
			...
			if (data.result == "success") {		
				...
				if(data.gyOperPay == ""){
					...
					window.top.WebPrint.setYffDataOperIdx2params(data.gyOperPay,window.top.WebPrint.nodeIdx.gymain);//打印用的参数
