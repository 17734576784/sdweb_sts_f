...
var misUseflag 	= "false", 	//是否使用MIS
	...
	misJezbs 	= 0,		//金额总笔数
	misHsdwbh	= null,		//--新增--电力资金结算单位编号
	misBatchNo	= null,		//--新增--电力资金编号（终端批次号）
	misDetailsvect = null;
	...
	$.post(def.basePath + "ajaxoper/actMisGy!taskProc.action", 
		{
			...
			zjgid 			: params.zjg_id,
			gyOperStr		: jsonString.json2String(params),
			...
			if(data.misUseflag == "true"){
				if(data.misOkflag == "true"){	//成功
					...
					$("#jsyhm").html("<a href='javascript:;' title='详细信息' onclick='mis_detaiInfo("+data.gyOperStr+",true)'>" + json.yhmc + "</a>");
					misJezbs	= json.jezbs;
					misDetailsvect = json.detail;
					
					//河南MIS没有客户分类
					if(window.top.provinceMisFlag == "HN"){
						$("#khfl").css("visibility","hidden");
						misHsdwbh	= json.hsdwbh;			//河南新增
						misBatchNo  = json.batch_no;		//河南新增
					}
					else{
...
function mis_detaiInfo(detail, flag){
	...
	if("HB" == provinceMisFlag){
		modalDialog.url = def.basePath + "jsp/spec/dialog/sg186Detail.jsp";
	}
	else if("HN" == provinceMisFlag){
		modalDialog.url = def.basePath + "jsp/spec/dialog/sg186DetailHN.jsp";
	}
	else{
		alert("不支持该省份MIS查询！");
		return;
	...
	$.post(def.basePath + "ajaxoper/actMisGy!taskProc.action", 
		{
			...
			zjgid 			: params.zjg_id,
			gyOperStr		: jsonString.json2String(params),
			userData2 		: ComntUseropDef.YFF_GYOPER_MIS_PAY
		}, function(data){
			loading.loaded();
				alert(data.misResult);
	});
	...
	$.post(def.basePath + "ajaxoper/actMisGy!taskProc.action", 
		{
			...
			zjgid 			: params.zjg_id,
			gyOperStr		: jsonString.json2String(params),
			userData2 		: ComntUseropDef.YFF_GYOPER_MIS_REVER
		},
	function(data){
		loading.loaded();
		alert(data.misResult);
...
function mis_detaiInfo(detail, flag){
	...
	if("HB" == provinceMisFlag){
		...
		modalDialog.width = 350;
		modalDialog.url = def.basePath + "jsp/spec/dialog/sg186DetailGS.jsp";
	}
	else{
		alert("不支持该省份MIS查询！");
		return;
...
function mis_pay(params){	//缴费
	...
	if(window.top.provinceMisFlag == "GS"){
		url = def.basePath + "ajaxoper/actMisGyGS!taskProc.action";
	}
	else{
		url = def.basePath + "ajaxoper/actMisGy!taskProc.action";
	}
