...
function selOneCons(id){//用户列表单击一行
	...
	dbRemain  = parseInt(jsonCons.rows[id].data[7]);
	dbRemainf = parseInt(jsonCons.rows[id].data[8]);
...
function setValue2Jsp(){//根据终端所选终端ID和总加组ID，初始化缴费部分。
	...
	if ((feectrl_type == SDDef.YFF_FEECTRL_TYPE_RTU) && (dbRemainf == 1)) {
		$("#now_remain2").html(round(rtnValue.now_remain2, def.point));
	...
	$.post(
		...
		function(data){
			...
			if(json.remain_val == ""){
				...
				if ((feectrl_type == SDDef.YFF_FEECTRL_TYPE_RTU) && (dbRemainf == 1)) {
					$("#zbje").val(round(parseFloat(dbRemain) - dbremain, def.point));
				}
				calcu();
