...
function initOrg(){
	$.post(def.basePath + "ajax/actCommon!getOrg.action",{},function(data){
		...
		if(data.result != ""){
			...
			if(!pos){
				if(data.flag != "1"){
					$("#org").append("<option value=-1>All</option>");
				}
			}
...
function getRtuByOrg(org_id){
	$("#rtu").html("<option value=-1>All</option>");
	if(org_id == -1){
		load_flag[1] = true;
		return;
	}
	$.post(def.basePath + "ajax/actCommon!getRtuByOrg.action",{value : org_id, appType : 3},function(data){
		load_flag[1] = true;
		if(data.result != ""){
			var json = eval('(' + data.result + ')');
			for(var i=0;i<json.length;i++){
