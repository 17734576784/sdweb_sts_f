...
var rtu_id = -1, mp_id = -1, op_type = -1, bczlsh = "";
...
function search() {
	...
	$("#total").html("");
	mygrid.clearAll();
	$.post(def.basePath + "ajaxoper/actMisDy!misErrSearch.action",{result : param},function(data){
		loading.loaded();
		if(data.result != ""){
			var json = eval('(' + data.result + ')');
...
function upload() {
	...
	if(param.op_type == SDDef.YFF_OPTYPE_REVER) {
		userData2 = ComntUseropDef.YFF_DYOPER_MIS_CHECKREVER;
	}
	else {
		userData2 = ComntUseropDef.YFF_DYOPER_MIS_CHECKPAY;
	}

	var str_url = "ajaxoper/actMisDy!taskProc.action";
	if (provinceMisFlag == "GS") {
		str_url = "ajaxoper/actMisDyGS!taskProc.action";
	}
	...
	$.post(def.basePath + str_url,
		{
			dyOperStr : jsonString.json2String(param),
			userData1 : rtu_id,
			userData2 : userData2
		},
	function(data){
