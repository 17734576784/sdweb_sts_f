...
function selcons(){//检索
	var tmp = doSearch("yc",ComntUseropDef.YFF_DYOPER_CHANGERATE,rtnValue);
	if(!tmp){
		return;
	}
	rtnValue = tmp;
...
function doSetFee(feeType, flag){//设置费率,feeType为费率类型，flag为批量设置标志
	...
	params.rated_z  = $("#fee_z").val();
	params.ratef_j  = $("#fee_j").val();
	params.ratef_f  = $("#fee_f").val();
	params.ratef_p  = $("#fee_p").val();
	params.ratef_g  = $("#fee_g").val();
