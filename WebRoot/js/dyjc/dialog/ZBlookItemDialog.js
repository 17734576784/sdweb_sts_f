...
function setCo(){
	...
	unHiddenCol = unHiddenCol.substring(1);//第一个是','所以要去掉 index为0的,
	unHiddenCol_tmp = unHiddenCol_tmp.substring(1);//第一个是','所以要去掉 下标为0的,
...
//	cookie.set(cookie.lookitem, alltypeCol);
	...
	if(item == alltypeCol){
		 ...
		 returnValues[0]   = "1" + alltypeCol;	//不同
		 returnValues[1]   = alltypeCol_tmp
		 window.returnValue = returnValues ; 
	}
	window.close();
