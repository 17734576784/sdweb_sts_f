...
function doSearch(type,operType,rtnValue){//打开检索页
	//type:   页面类型：卡式-ks/远程-yc/主站-zz;卡式外接-kzext(20140606添加)
...
//				对于没有定义ComntUseropDef.YFF_DYOPER_xxx的页面,operType给传个ComntUseropDef之外的任意整数值即可。
	...
	var url = def.basePath + "jsp/dyjc/dialog/selCons.jsp";
	if(type == 'kzext'){
		url = def.basePath + "jsp/dyjc/dialog/selConsExt.jsp";
	}
...
function importBD(rtuId,mpId){//录入表底
	...
	modalDialog.url = def.basePath + "jsp/dyjc/dialog/importBD.jsp";
	var o = modalDialog.show();
	if(o!=undefined || o!=null){
		return o;		
	}else{
