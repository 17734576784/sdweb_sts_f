...
function initgrid(){
	...
	var empty = "&nbsp;";
	var footer = "<div id='hj'>Total：</div>,#cspan,#cspan,<div id='jfcs'>"+empty+"</div>,<div id='jfje'>"+empty+"</div>,<div id='zje'>"+empty+"</div>,<div id='zdl'>"+empty+"</div>,&nbsp;,#cspan";
	...
	$("#filename").val(encodeURI("Résumé d'achat de la lecture du compteur"));
...
function search(){
	...
	$.post(def.basePath + "ajaxdyjc/actReport!rtucoltmoney.action",{result : param},function(data){
		loading.loaded();
		if(data.result != ""){
...
//			$("#jsje").html(hz.jsje);
			...
			var footerType = "str,int,0.000,0.000,0.000";
			$("#footer").val(footer);
