...
function selcons(){//检索
	...
	var js_tmp = doSearch("kzext",ComntUseropDef.YFF_DYOPER_REVER,rtnValue);
	if(!js_tmp)	return;
	rtnValue = js_tmp;
...
function calcu(){
	...
	else {//	alert("rtnValue.jt_total_dl- rtnValue.buy_dl="+ parseFloat(rtnValue.jt_total_dl)+  "  " + parseFloat(rtnValue.buy_dl))
		var retcalc = CalcBuyDl(rtnValue.feeType, zje, dj_str, parseFloat(rtnValue.jt_total_dl)- parseFloat(rtnValue.buy_dl));
		if (retcalc.retf == -1) {
			tmp_dl  = zje / dj;
		}
		else {
...
function read_card(){//读卡
	...
	loading.loading();
	window.setTimeout("window.top.card_comm.readExtCardSearchDy(" + ComntUseropDef.YFF_DYOPER_REVER + ")", 10);
}
...
function readCard_search(){
	...
	//如果为多个用户使用，则从模态窗口中选择。
	window.top.card_comm.readExtCardSameWrtNo(ComntUseropDef.YFF_DYOPER_REVER);
}
...
//	params.meter_type 	= WRITECARD_TYPE;    ???
	...
	params.buy_dl		= params.buy_dl == '' ?  0 : params.buy_dl;
	params.pay_bmc		= $("#pay_bmc").html();
	params.pay_bmc		= params.pay_bmc == '' ? 0 : params.pay_bmc;
	...
	if ((params.cardtype == "JJ_003") || (params.cardtype == "JJ_004")){
		...
		params.buynum		= Number(params.buynum) -1;							//江机上次补卡 之后特殊修正， 之后购电
		params.buy_dl		= 0;
		params.pay_bmc		= 0;
	}
	...
	$.post( def.basePath + "ajaxoper/actWebCard!getWriteCardExt.action", 		//后台处理程序
		...
		function(data) {			    	//回传函数
			...
			if(ret_json.errno  == 0){
				...
				params.buynum  = Number(params.buynum) + 1;
				wrtDB_cz(params);
			}else{
				alert("写卡失败！\n" + ret_json.errsr);
				loading.loaded();
...
//		}
	...
	//	buyTime		= json_out.back_pay_num;//返写购电次数	
		...
		else if (Number(rtnValue.buy_times) !=Number(wbuyTime)) {	
